...
class OrderLine extends BaseResource {
   ...
   */
  public async function updateAsync(): Awaitable<BaseResource> {
    $url = 'orders/' . $this->orderId . '/lines/' . $this->id;
    $body = json_encode($this->getUpdateData());
    $result = $this->client->performHttpCallAsync(HttpMethod::PATCH, $url, $body);
  ...
  public function assert(
    ...
    if(C\contains_key($datas, 'vatRate') && $datas['vatRate'] !== null) {
